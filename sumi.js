(() => {
 ...
 float pigment=smoothstep(.06,.52,broad+.16*middle);
 vec3 ink=vec3(.032,.028,.027),bone=vec3(.29,.18,.15),red=vec3(.56,.018,.054);
 float redZone=smoothstep(.15,.6,sin(field*6.8+1.4)+.25*cos(p.y*2.));
 vec3 dye=mix(vec3(.16,.018,.027),red,.38+.62*redZone);
 ...
 col+=thin*vec3(.10,.022,.021);
 float hairline=(1.-smoothstep(.005,.025,abs(broad+.16*middle-.34)))*smoothstep(.60,.93,sin(field*4.1));
 col=mix(col,bone,hairline*.6);
