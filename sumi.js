(() => {
 ...
 function draw(){if(!gl||lost)return;if(reduced.matches||navigator.connection?.saveData){host.dataset.render='fallback';return;}gl.uniform2f(uniforms.res,canvas.width,canvas.height);gl.uniform1f(uniforms.time,elapsed);gl.drawArrays(gl.TRIANGLES,0,6);host.dataset.render='webgl';canvas.dataset.frames=String(++drawn);}
 ...
 function tick(now){raf=0;if(!allowed()){sync();return;}if(!last)last=now;const delta=now-last;if(delta>=32){if(delta>50)slow++;else slow=Math.max(0,slow-1);elapsed+=Math.min(delta,75)/1000;last=now;if(slow>50&&quality>.65){quality=.65;resize();slow=0;}draw();}raf=requestAnimationFrame(tick);}
